...
var isAudioFileToPlay=false;
var audioFileLoc="";
var audioFileContext;
var updateAudioFileInterval; 
...
function toggleMono() {
	...
	if(isLaunched && isAudioFileToPlay && audioInput != null)
	{console.log(audioInput )
           
audioFileContext.currentTime = 0; 
audioFileContext.stop();
    ...
    if (audioInput != realAudioInput) {
		...
		if(realAudioInput != null){ // added as fix on 16/02/2021
        realAudioInput.disconnect();
        audioInput = realAudioInput;}
    } else {
        if(realAudioInput != null){realAudioInput.disconnect();
	...
	isLaunched=false; // added as fix on 16/02/2021
  initAudio();
     // document.getElementById("effect").selectedIndex=17;
  //setTimeout(function(){changeInput();}, 300);
}

var useFeedbackReduction = true;
function gotStream(stream) {
	
	if(isAudioFileToPlay) //REF: https://www.creativebloq.com/advice/get-started-with-the-web-audio-api
	{
		    var irRRequest = new XMLHttpRequest();

			    irRRequest.open("GET",audioFileLoc, true);// "/sounds/c.mp3", true);
    irRRequest.responseType = "arraybuffer";
    irRRequest.onload = function(){
        audioContext.decodeAudioData(irRRequest.response, onDecoded);
    }
    
    function onDecoded(buffer){
        audioFileContext = audioContext.createBufferSource();
        audioFileContext.buffer = buffer;
        audioFileContext.connect(audioContext.destination);
		        audioFileContext.playbackRate.value = 1.0;//between 0.0 & 10.0 FMI: http://man.hubwiz.com/docset/JavaScript.docset/Contents/Resources/Documents/developer.mozilla.org/en-US/docs/Web/API/AudioBufferSourceNode/playbackRate.html
        audioFileContext.loop = true; 
		audioInput=audioFileContext
        audioInput.start();
	...
	/*
updateAudioFileInterval=	setInterval(function(){  
    document.getElementById("ssRangeSeekControl").innerHTML =  Math.floor(audioContext.currentTime).toString() ; 
    document.getElementById("isRangeSeekControl").innerHTML = Math.floor(audioContext.currentTime).toString() ;
	      document.getElementById("isRangeSeekControl").value =  Math.floor(audioContext.currentTime).toString() ;
		document.getElementById("isRangeSeekControl").max = Math.floor(audioContext.currentTime).toString() ;//meta data load, how in this case? https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/loadedmetadata_event 
...
var recorder;
function startRecordAudio()//REF: https://hacks.mozilla.org/2016/04/record-almost-everything-in-the-browser-with-mediarecorder/
{//var filter = audioContext.createBiquadFilter();
	var streamDestination = audioContext.createMediaStreamDestination();
//filter.connect(streamDestination);
/*if(isAudioFileToPlay){audioFileContext.connect(streamDestination);//	audioInput.connect(streamDestination);// only playback?
}
else{
	audioInput.connect(streamDestination);
}*/
    if (document.getElementById('chAllowEffectsRecord').checked) {
outputMix.connect(streamDestination); 	// only the processed parts
	}
	    if (document.getElementById('chAllowOriginRecord').checked) {
audioInput.connect(streamDestination);// only your audio (file/ mic).
}
...
function initAudioFILE(){
 ...
 var  fr = new FileReader();
  fr.readAsDataURL(document.getElementById("iUploadAudio").files[0]);
  fr.onload = e => {
	  audioFileLoc= e.target.result;
   /* var audio = document.createElement('audio');
    audio.src = e.target.result;
    var context = new(window.AudioContext || window.webkitAudioContext)(),
      source = context.createMediaElementSource(audio);
    console.log(source);*/
	isAudioFileToPlay=true;
		initAudio();

  };
}
 	function playbackControlChange(feedbackVal){
			if(audioFileContext != null){
   audioFileContext.playbackRate.value = parseFloat( feedbackVal);
  document.getElementById("sRangeAudioFilePlayBackSpeed").innerHTML =  ( feedbackVal).toString();
    document.getElementById("iRangeAudioFilePlayBackSpeed").innerHTML =  ( feedbackVal).toString();

  }
}

/*
 	function seekTimeChange(feedbackVal){
			if(audioFileContext != null){  
     audioContext.currentTime.value = parseFloat( feedbackVal);
     document.getElementById("ssRangeSeekControl").innerHTML =  feedbackVal ; 
    document.getElementById("isRangeSeekControl").innerHTML = feedbackVal ;
	      document.getElementById("isRangeSeekControl").value =feedbackVal ;
 document.getElementById("isRangeSeekControl").defaultValue = feedbackVal;
 //TBC:  //REF:https://stackoverflow.com/questions/18661296/webaudio-not-playing-after-stop0
